...
本仓库也可作为 [`Furinelle/furina`](https://github.com/Furinelle/furina) 的本地原神资料缓存使用。`furina` 的 wiki 工具会读取本仓库中的原神 Markdown 文档，用于补查角色资料库未覆盖的剧情、任务、语音、角色逸闻等内容。

推荐把两个仓库放在同一个父目录下：

```bash
git clone https://github.com/Furinelle/furina.git
...
在这种布局下，`furina` 会默认连接到 `../genshinstory-cache`，实际读取路径为：

```text
genshinstory-cache/web/docs-site/public/domains/gi/docs
```

如果本仓库不在 `furina` 的同级目录，可以在运行 `furina` 的 wiki 工具时设置 `GENSHIN_STORY_ROOT`，或传入 `--root <本仓库路径>` 指向本仓库根目录。该连接只需要本地文件可读，不需要启动本仓库的前端或后端服务。
